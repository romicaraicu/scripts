...
#         
#
# AUTHOR:      Hinnerk Rüf (hinnerk.ruemenapf@rrz.uni-hamburg.de)
#              Based on work by 
...
# Hinnerk Rüf, Trond H. Amundsen, Gunther Schlegel, Matija Nalis, 
...
#18. RedHat 4 sample, (thanks to Séstien Maury)
...
#19. RedHat 4 sample, modified (thanks to Séstien Maury)
"MYVOLUME (36005076801810523100000000000006f)\n"
."[size=576 GB][features=\"1 queue_if_no_path\"][hwhandler=\"0\"]\n"
."\\_ round-robin 0 [active]\n"
." \\_ 13:0:1:0  sdc 8:32  [active]\n"
...
foreach my $lunName ( sort {$a cmp $b} keys %lunPaths) {
  ...
  #print "$lunName: $pathCount  STANDARD\n";
    ...
    if ($pathCount < $crit){
  report("LUN $lunName: less than $crit paths ($pathCount/$warn)! | returncode=$E_CRITICAL" , $E_CRITICAL);
    } elsif ($pathCount < $warn){
  report("LUN $lunName: less than $warn paths ($pathCount/$warn). | returncode=$E_WARNING", $E_WARNING);
    } else {
  report("LUN $lunName: $pathCount/$warn. | returncode=$E_OK", $E_OK);
    }
} # foreach
